...
from nncd.batch.s3_csv_files import LocalFile
...
class ConvertStatus:
    ...
    def write(self, progress: int=None):
        ...
        res = Datasets.update(update_data).where(
                Datasets.dataset_id == self.dataset_id,
                Datasets.deleted == False).execute()
        if not res:
...
class IndexCsvFile(LocalFile):
    def __init__(self, tenant_id, dataset_id):
        super().__init__(consts.DATASETS_DIR_NAME, consts.DATASET_OUTPUT_CSV)
...
@retry(stop_max_attempt_number=consts.MAX_RETRY_CNT)
def init_status(cache_dir: str, dataset_dir: str):
    logger.info('[BEGIN] "Initinalize cache file {}"'.format(cache_dir))

    status = ConvertStatus(cache_dir)
    status.write(progress=0)
    try:
        index_csv_path = os.path.join(dataset_dir, consts.DATASET_OUTPUT_CSV)
...
def create_dataset_cache(status, cache_dir, dataset_dir):
    ...
    try:
        console_cli_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "console_cli.py")
        cmd = [sys.executable,console_cli_path,
               'conv_dataset', '-F', f'{dataset_dir}/{consts.DATASET_OUTPUT_CSV}', cache_dir]
        ...
        status.update_error(Code.E_NNCD_NNABLA_ERROR.value, f'Failed to create dataset cache.')
        status.failed()
        status.write()
        raise
    finally:
...
def create_dataset_page_data(status, dataset_dir):
    logger.info("[Create page files for dataset] '{}/{}'".format(status.tenant_id, status.dataset_id))
        
    # create page data
    csv_object = IndexCsvFile(status.tenant_id, status.dataset_id)
    ...
    try:
        ...
        if is_delete:
            ...
            for line in UrlEmbedder.embed(
                csv_reader, None, None,
                status.tenant_id, status.dataset_id, PathType.LOCAL
            ):
...
def main(argv: list):
    ...
    try:
        ...
        if not dataset_id or not tenant_id:
            raise BaseError('Not `dataset_id` or `tenant_id` arg.',
                            Code.E_NNCD_ARGS_ERROR)

        cache_dir = os.path.join(consts.CACHE_DIR_NAME, tenant_id, str(dataset_id))
        dataset_dir = os.path.join(consts.DATASETS_DIR_NAME, tenant_id, str(dataset_id))
        
        # init status
        status = init_status(cache_dir, dataset_dir)

        # create dataset cache
        create_dataset_cache(status, cache_dir, dataset_dir)
        
        # create page file
        create_dataset_page_data(status, dataset_dir)
