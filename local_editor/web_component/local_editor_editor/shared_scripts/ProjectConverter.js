...
(() => {
    ...
    var _SVC_GRID = 20; // XXX avoid reference of nnc.Definitions.EDIT.GRID.SIZE for script's independency.
    var _APP_GRID = 24;
    ...
    importLayer = (layer_content, parseDatasetUri) => { // set global
        var project = {objects: function(section, number) {
            var array = [];
    ...
    importProject = (project_content, parseDatasetUri) => { // set global
        ...
        return {
            ...
            structure_search: {
                ...
                attempt_hours: structure_search.AttemptHours || nnc.emptyConfiguration.structure_search.attempt_hours,
                attempt_times: structure_search.AttemptTimes || nnc.emptyConfiguration.structure_search.attempt_times,
                num_parallel: structure_search.NumParallel || nnc.emptyConfiguration.structure_search.num_parallel,
            },
    ...
    exportLayer = (network) => { // set global
    ...
    exportProject = (configuration, is_create_job, is_exclude_file_path) => { // set global
        ...
        return [
            ...
            "AttemptHours=" + (configuration.structure_search.attempt_hours || nnc.emptyConfiguration.structure_search.attempt_hours),
            "AttemptTimes=" + (configuration.structure_search.attempt_times || nnc.emptyConfiguration.structure_search.attempt_times),
            "NumParallel=" + (configuration.structure_search.num_parallel || nnc.emptyConfiguration.structure_search.num_parallel),
        ].join('\r\n');
    };

    getComponentType = (project_content) => { // set global
        var project = {objects: function(section, number) {
            var array = [];
            number = Number(number);
            for (var i = 0; i < number; ++i) {
        ...
        var propertiesOf = function(layer) {
            ...
            var _parseType = type => type === 'File' ? 'Text' : type;
            var res = {};
