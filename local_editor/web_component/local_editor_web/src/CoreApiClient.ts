...
export class CoreApiClient {
  ...
  public static async getInstance(userId: string, tenantId: string): Promise<IInstanceType[]> {
    const url: string = `${CoreDomain}v1/users/${userId}/tenants/${tenantId}/instance`;
    const response: any = await axios.get(url, { withCredentials: true });
    return response.data.instance_type.train.filter((instance: IInstanceType) => instance.available && !instance.deprecated)
  }

  public static async createInstance(userId: string, tenantId: string, instance: IremoteResGForm): Promise<IInstanceType[]> {
    const url: string = `${CoreDomain}v1/users/${userId}/tenants/${tenantId}/instance`;
    const headers: { 'Content-Type': string } = {
      'Content-Type': 'application/json'
    };
    const response: AxiosResponse<IGetInstance> = await axios.post(url, instance, { headers, withCredentials: true });
    return response.data.instance_type.train.filter((instance: IInstanceType) => instance.available && !instance.deprecated)
  }
  ...
  * @param projectIDs
  */
  public static deleteInstances(userId: string, tenantId: string, instanceIds: number[]): Promise<any> {
    const url: string = `${CoreDomain}v1/users/${userId}/tenants/${tenantId}/instance`;
    return new Promise((resolve: (res: any) => void, reject: (res: any) => void) => {
      const url: string = `${CoreDomain}v1/users/${userId}/tenants/${tenantId}/instance`;
      const headers: { 'content-type': string } = {
      ...
      axios.delete(url, { headers, withCredentials: true, data: {
        ids: instanceIds
      } }).then((res: any) => {
        resolve(res.data.instance_type.train.filter((instance: IInstanceType) => instance.available && !instance.deprecated));
      }, (res: any) => {
        reject(res);
      });
    });
  ...
  public static getBackendEnvInfo(): Promise<any> {
    return new Promise((resolve: (res?: any) => void, reject: (res?: any) => void) => {
      const url: string = `${CoreDomain}v1/misc/env_info`;
      const headers: { 'Content-Type': string } = {
        'Content-Type': 'application/json'
      };
      axios.get(url, { headers, withCredentials: true }).then((res: any) => {
  ...
  /*nncd: add api uploadDataset, getDatasetSaveDir*/
  /**
   * send the absolute path of the dataset directory
   * designed for desktop app only
   * @param userID
   * @param datasetPath
   */
  public static uploadDataset(userId: string, tenantId?: string, datasetPath?: string): Promise<any> {
    return new Promise((resolve: (res?: any) => void, reject: (res?: any) => void) => {
      const url: string = `${CoreDomain}v1/users/${userId}/datasets/upload`;
      const headers: { 'Content-Type': string } = {
        'Content-Type': 'application/json'
      };
      const queryParams: any = {
  ...
  /**
   * get dataset save dir
   * designed for desktop app only
   * @param userID
   */
  public static getDatasetSaveDir(userId: string, tenantId?: string): Promise<any> {
    return new Promise((resolve: (res?: any) => void, reject: (res?: any) => void) => {
      const url: string = `${CoreDomain}v1/users/${userId}/datasets/save_dir`;
      const headers: { 'Content-Type': string } = {
        'Content-Type': 'application/json'
      };
      const queryParams: any = {
