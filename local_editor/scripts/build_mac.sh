...
cp -rf ~/.anyenv/envs/pyenv/versions/3.10.14 electron_app/python_bundles || exit 1

# # copy lib
cp -P /usr/local/opt/gettext/lib/libintl.8.dylib electron_app/python_bundles/bin/ || exit 1
cp -P /usr/local/opt/openssl@1.1/lib/libssl.1.1.dylib electron_app/python_bundles/lib/python3.10/lib-dynload/ || exit 1
cp -P /usr/local/opt/openssl@1.1/lib/libcrypto.1.1.dylib electron_app/python_bundles/lib/python3.10/lib-dynload/ || exit 1

install_name_tool \
    -change /usr/local/opt/gettext/lib/libintl.8.dylib "@executable_path/libintl.8.dylib" \
    electron_app/python_bundles/bin/python3.10 || exit 1
install_name_tool \
...
install_name_tool \
    ...
    electron_app/python_bundles/lib/python3.10/lib-dynload/libssl.1.1.dylib || exit 1

# # pip install
electron_app/python_bundles/bin/python3.10 -m pip install --upgrade pip || exit 1
electron_app/python_bundles/bin/python3.10 -m pip install nnabla==${NNABLA_VERSION} || exit 1
electron_app/python_bundles/bin/python3.10 -m pip install nnabla_converter==${NNABLA_VERSION} || exit 1
